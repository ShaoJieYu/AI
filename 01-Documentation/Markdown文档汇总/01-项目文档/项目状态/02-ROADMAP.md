...
**文档版本**: v3.4 | **更新时间**: 2026-07-01

> 📋 本文档是项目核心任务线：把"调 AI API 生成备课内容"的工具升级为真正的 AI Agent 系统
> **当前进度**: ✅ 四阶段全部完成（Function Calling + RAG/记忆 + Planning + Multi-Agent）+ 阶段 4 质量优化 + 性能优化 + RAG 检索准确性修复 + 物理教材入库与跨学科隔离 + Unit 参数三层保障 + 本地模型接入（双 Provider 可切换）+ 前端模型切换功能（运行期热切换）+ **内容生成并发数动态化重构（2026-07-01）**
...
### 本地模型接入（2026-06-30 完成）

**目标**：把"只能调云端 API"的工具升级为"云端 + 本地双 Provider 可切换"的灵活架构，开发调试用本地省 token，正式演示用云端保质量。

**背景**：用户硬件为 RTX 4060 Laptop 8GB 显存 + 16GB 内存，可以跑 4-bit 量化的 7B 模型（如 Qwen2.5-7B，4.36 GiB）。

**技术选型**：
| 能力 | 选型 | 理由 |
|------|------|------|
| 本地推理引擎 | **Ollama** | Windows 原生支持，一键拉模型，自带 OpenAI 兼容 REST API |
| 本地模型 | **Qwen2.5-7B-Instruct** | 与云端 qwen-plus 同系列，prompt 兼容性最好，原生支持 Function Calling |
| Python 客户端 | **openai SDK** | 通过 Ollama 的 `/v1/chat/completions` 调用，与 OpenAI 格式一致 |

**实现方案（双 Provider 工厂模式）**：

**第 1 步 - LLM 层抽象**：
- `common/llm.py` 新增 `BaseLLM` 抽象基类，定义 `chat()` 和 `chat_with_tools()` 两个方法
- `QwenPlusLLM`（云端，dashscope SDK）和 `OllamaLLM`（本地，openai SDK）都继承 `BaseLLM`
- `get_llm()` 工厂函数读 `config.LLM_PROVIDER` 返回对应实例
- 全局单例 `llm` 替代原 `qwen_plus`，保留 `qwen_plus = llm` 别名向后兼容

**第 2 步 - 配置切换**：
- `.env` 新增 `LLM_PROVIDER=cloud`（默认）/ `local`（本地模型）
- `OLLAMA_BASE_URL=http://127.0.0.1:11434/v1`
- `OLLAMA_MODEL=qwen2.5:7b`

**第 3 步 - Ollama 安装到 D 盘**：
- 程序：`D:\Ollama\bin\ollama.exe`（v0.3.11）
- 模型：`D:\Ollama\blobs\`（4.7GB Qwen2.5-7B）
- 环境变量：`OLLAMA_MODELS=D:\Ollama`（Machine + User 双级设置，避免回退到 C 盘）
...
| 测试项 | 结果 |
|---|---|
| 工厂函数选择 | ✓ 正确返回 OllamaLLM |
| chat() 纯文本对话 | ✓ 正常回复 |
| chat_with_tools() 工具调用 | ✓ LLM 自主调用 search_textbook |
| response_format JSON 输出 | ✓ 标准格式 JSON |

E2E 验证（本地模型跑通完整备课流程）：
| 指标 | 云端 qwen-plus | 本地 Qwen2.5-7B |
|---|---|---|
| 备课流程 | ✓ 跑通 | ✓ 跑通 |
| 响应时间 | 78s | 167s |
| Token 成本 | 按 token 计费 | **零成本** |
| 模型加载 | API 即用 | 29/29 层 GPU offload，4168 MiB 显存 |
| 数据隐私 | 数据上云 | **完全本地** |

**关键经验**：
- Ollama 在 Windows 上默认装 C 盘，必须显式设置 `OLLAMA_MODELS` 环境变量并清理 C 盘残留
- PowerShell Machine 级环境变量需要管理员权限，User 级足够且更安全
- openai SDK 比 ollama-python SDK 更稳定，且未来切到其他推理框架（vLLM）时通用性最好
- 本地 7B 模型在备课这种长文本生成任务上质量略逊于云端 qwen-plus，但开发调试足够用，正式演示仍推荐切回云端
...
### 前端模型切换功能（2026-06-30 完成）

**目标**：把"改 .env + 重启 AI 服务"才能切换 LLM 的部署期操作，升级为"前端设置页点一下"的运行期热切换，提升演示灵活性和用户体验。

**背景**：本地模型接入后，切换 cloud/local 必须改 `.env` 文件并重启 8001 端口的 AI 服务，每次切换耗时 5-10 秒且中断所有正在进行的请求。毕设答辩演示时需要快速对比云端和本地效果，这个体验不可接受。

**实现方案（注册表模式 + 代理对象）**：

**第 1 步 - LLMRegistry 注册表**：
- `common/llm.py` 新增 `LLMRegistry` 类，内部持有可变 `_current` 引用，`switch(provider)` 替换实例
- 线程安全：`threading.Lock` 保护 `_current` 切换
- 持久化：provider 写入 `.runtime_llm.json`，AI 服务重启后读取该文件（不依赖 .env 默认值）

**第 2 步 - 代理对象向后兼容**：
- `_LLMProxy` 类所有方法（chat / chat_with_tools）转发到 `registry.get()` 当前实例
- 全局 `llm = _LLMProxy()` 替代原固定单例
- 7 处 `from common.llm import llm` 调用方代码**零改动**（代理对象透明转发）

**第 3 步 - AI 服务 HTTP 接口**：
- `GET /api/llm/provider` — 返回当前 provider + 可用列表
- `POST /api/llm/provider` — 热切换（body: {provider: "cloud"|"local"}）
- `GET /api/llm/status` — 健康检查（local 探活 Ollama /api/tags，cloud 检查 API key）

**第 4 步 - 后端 Java 转发**：
- `LlmProviderController` + `AiService` 3 个方法
- 走统一 `/api` 网关，JWT 鉴权（未带 token 返回 403）

**第 5 步 - 前端 UI**：
- 设置页新增第 4 个 Tab「AI 模型」（RobotOutlined 图标）
- 当前模型卡片：label + description + 连通状态 Tag（绿色已连接/红色未连接）
- Radio.Group 切换器：cloud / local 两个选项，选中非当前项显示「切换到该模型」按钮
- React Query 数据加载 + useMutation 切换 + invalidateQueries 刷新
...
|---|---|
| 初始化读取持久化/.env 默认值 | ✓ |
| switch("local") 实例类型变为 OllamaLLM | ✓ |
| switch("cloud") 实例类型变为 QwenPlusLLM | ✓ |
| 同名 provider 切换幂等 | ✓ |
| 非法 provider 返回 False | ✓ |
| 持久化文件正确写入 | ✓ |
| 重启后读取持久化 provider | ✓ |
| 代理对象转发到当前实例 | ✓ |
| 代理对象方法签名正确 | ✓ |
...
| 验证项 | 结果 |
|---|---|
| AI 服务 GET /api/llm/provider | ✓ 返回 local + 可用列表 |
| AI 服务 GET /api/llm/status（local） | ✓ Ollama 服务在线 |
| AI 服务 POST 切换到 cloud | ✓ success: true |
| AI 服务 POST 非法 provider | ✓ 返回 400 |
| 后端网关 GET（带 token） | ✓ Result 包装格式正确 |
| 后端网关 POST 热切换 | ✓ 转发到 AI 服务成功 |
| 后端网关未带 token | ✓ 返回 403（JWT 拦截） |

**关键经验**：
- 代理对象模式是"改核心不改调用方"的最佳实践：7 处调用方代码零改动，热切换立即对所有调用方生效
- 持久化用 JSON 文件而非数据库：AI 服务是独立 Python 进程不连 MySQL，文件读写最简单，且只存一个字符串没必要引入数据库依赖
- 健康检查接口区分 provider：local 实际探活 Ollama 端点，cloud 只检查 API key 配置（不实际调用避免耗 token）
- 后端做转发层而非前端直连 AI 服务：保持现有 `/api` 网关架构一致，JWT 鉴权统一在 Java 层处理
